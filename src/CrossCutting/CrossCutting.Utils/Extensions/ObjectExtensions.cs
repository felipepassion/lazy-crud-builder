    ...
    public static T Update<T, K>(this T item, K other, params string[] namesToIgnore)
    {
        ...
        foreach (FieldInfo fi in fis)
        {
            ...
            if (fi.FieldType.Namespace != item.GetType().Namespace)
            {
                ...
                {
                    var myArr = myVal as byte[];
                    var otherArr = otherVal as byte[];
                    if (myArr == null || !myArr.SequenceEqual(otherArr))
                    {
                        fi.SetValue(item, otherArr);
                    }
                }
                else if ((myVal == null && otherVal != null) || myVal?.ToString()?.Equals(otherVal?.ToString(), StringComparison.InvariantCultureIgnoreCase) == false)
                {
